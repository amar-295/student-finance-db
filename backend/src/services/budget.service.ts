 ...
 */
const calculateBudgetStatus = async (
  userId: string,
  budget: any,
): Promise<BudgetStatus> => {
...
export const getBudgetRecommendations = async (
  ...
  const categorySpending: Record<
    string,
    { total: number; months: Set<string>; categoryId: string }
  > = {};
...
const calculateBudgetPeriod = (
  ...
  switch (period) {
    ...
    case "semester":
      ...
      startDate = new Date(
        Date.UTC(now.getUTCFullYear(), semesterStart, 1, 12, 0, 0),
      );
      endDate = new Date(
        Date.UTC(now.getUTCFullYear(), semesterStart + 6, 0, 12, 0, 0),
      );
      break;
