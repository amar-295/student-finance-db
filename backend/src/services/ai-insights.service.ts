 ...
 */
const analyzeSpendingPatterns = async (userId: string): Promise<SpendingData> => {
    const now = new Date();
    ...
    transactions.forEach((t) => {
        ...
        categoryBreakdown[category] = (categoryBreakdown[category] || 0) + Math.abs(Number(t.amount));
    });
    ...
    const lastWeekTransactions = transactions.filter(
        (t) => t.transactionDate >= startOfLastWeek && t.transactionDate < startOfWeek
    );
...
const generateStatisticalInsights = (data: SpendingData): Insight[] => {
    ...
    const weekChange = ((data.weekComparison.thisWeek - data.weekComparison.lastWeek) /
        data.weekComparison.lastWeek) * 100;
            ...
            return [
                {
                    ...
                    message: aiText.replace(`Financial advice for a student: ${summary}. Tip:`, '').trim(),
                    priority: 4,
                },
            ];
        }
...
const getSavingTip = (category: string, amount: number): string => {
    const tips: Record<string, string> = {
        ...
        'Shopping': 'Wait 24 hours before buying non-essentials to avoid impulse purchases.',
        'Entertainment': 'Look for student discounts on streaming services and movies.',
        'Groceries': 'Making a shopping list and buying in bulk can reduce costs by 15-20%.',
    };

    return tips[category] || 'Track this category closely to identify savings opportunities.';
};
