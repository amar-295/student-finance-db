...
export const calculateHealthScore = async (userId: string) => {
  ...
  const totalScore = Math.round(
    budgetScore * 0.4 + savingsScore * 0.3 + spendingScore * 0.3,
  );
...
export const getRecommendations = async (userId: string) => {
  ...
  if (diningCategory) {
    ...
    if (Number(diningSpend._sum?.amount || 0) > 200) {
      recs.push({
        ...
        description:
          "You spent over $200 on dining last month. Cooking 2 more meals at home could save you ~$80.",
        impact: "$80/month",
      });
    }
  }
  ...
  recs.push({
    ...
    description:
      "Start small and save incrementally. Potential savings by year end: $1,378.",
    impact: "$1,378/year",
  });
