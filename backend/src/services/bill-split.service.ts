...
export const createBillSplit = async (
  ...
  if (input.groupId) {
    ...
    if (!isMember) {
      throw new ForbiddenError(
        "You must be a member of the group to create a split in it",
      );
    }
  }
  ...
  if (input.splitType === "equal") {
    const amountPerPerson =
      Math.floor((input.totalAmount / input.participants.length) * 100) / 100;
    let runningSum = 0;
    ...
    const participantsTotal = processedParticipants.reduce(
    ...
    if (Math.abs(participantsTotal - input.totalAmount) > 0.05) {
      throw new BadRequestError(
        `Participants total (${participantsTotal}) must match bill total (${input.totalAmount})`,
      );
    }
  }
 ...
 */
export const settleParticipantShare = async (
  userId: string,
  splitId: string,
  targetParticipantUserId: string,
) => {
  ...
  if (!split) {
    throw new NotFoundError(
      "Split not found or you do not have permission to settle it",
    );
  }
 ...
 */
export const addSplitComment = async (
  userId: string,
  splitId: string,
  content: string,
) => {
